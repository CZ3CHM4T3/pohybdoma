...
## 4b) KOMUNITA & NÁVYK (rozjeté)
- [x] **Chlubírna** – nástěnka pokroku (MEMBER+): text + fotka + „Povzbudit". `/chlubirna` (chlubirna.sql). Dlaždice v účtu.
- [x] **Síň slávy + odznaky (Fáze 1)** – `/sin-slavy`: vitrína (získané barevně / zamčené šedě + progress), přišpendlení až 3 odznaků (badges.sql), zobrazení u jména (účet, Chlubírna). Auto-udělování z dat (lekce, deník, oblíbená, buddies, chlubírna, výzvy, kruh, věrnost).
- [ ] 💡 Odznaky – doplnit měření pro: série dní, % knihovny, živé lekce, kurzy, kalkulačka, Mixér, Cestovatel úrovněmi; zobrazit pins i v Klubu/Kruzích/Buddies; oslava (konfety) + kupóny za milníky
- [x] **Buddies + 1:1 chat** – přátelé (žádost→přijetí) + soukromý chat naživo (Supabase Realtime). `/buddies` (buddies.sql). Dlaždice v účtu. *(online indikátor lze přidat později)*
