...
declare
  ...
  cli text; wd smallint; tm text; datum text; den text; cli_email text;
...
begin
  ...
  -- Klientovi (pokud má účet): potvrzení, že omluva prošla
  if new.cancelled_by is not null then
    select email into cli_email from public.profiles where id = new.cancelled_by;
